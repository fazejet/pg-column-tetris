...
export interface ParsedColumn {
  /** Column name as written in source (no quotes stripped for display) */
  name: string;
  /** Raw type string, e.g. "varchar(255)" or "timestamp with time zone" */
  type: string;
  /** Full text of the column definition line (for rewrites) */
  rawText: string;
  /** Character offset of the start of this column def in the file */
  startOffset: number;
  /** Character offset just past the end of this column def in the file */
  endOffset: number;
}
...
export interface ParsedTable {
  ...
  createOffset: number;
  /** Offset of `(` opening the column list */
  openParenOffset: number;
  /** Offset of `)` closing the column list */
  ...
  while (i < out.length) {
    ...
    if (c === "'") {
      ...
      while (i < out.length && out[i] !== "'") {
        if (out[i] === '\\' && i + 1 < out.length) i++; // skip escaped
        i++;
      }
      i++; // closing quote
      continue;
 ...
 */
function splitTopLevelCommas(body: string): { text: string; offset: number }[] {
 ...
 */
function isTableConstraint(segment: string): boolean {
 ...
 */
function parseColumnDef(segment: string): { name: string; type: string } | null {
...
function parseTableHeader(
  source: string,
  createIdx: number,
): { tableName: string; openParenOffset: number } | null {
  // Skip past CREATE  [TEMP|TEMPORARY|UNLOGGED]?  TABLE  [IF NOT EXISTS]?  name (
  const headerRe =
    /CREATE\s+(?:GLOBAL\s+|LOCAL\s+)?(?:TEMP(?:ORARY)?\s+|UNLOGGED\s+)?TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([A-Za-z_"][\w".]*)\s*\(/iy;
 ...
 */
export function parseCreateTables(originalSource: string): ParsedTable[] {
  ...
  while ((m = createRe.exec(source)) !== null) {
    ...
    for (const seg of segments) {
      if (isTableConstraint(seg.text)) continue;
