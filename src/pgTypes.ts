...
export interface PgType {
  ...
  align: number;
  /** storage size in bytes; -1 = variable-length */
  size: number;
  /** true for variable-length types like text/varchar */
  variable: boolean;
}
 ...
 */
export const PG_TYPES: Record<string, PgType> = {
 ...
 */
export function resolveType(rawType: string): PgType | null {
  ...
  if (dotIdx >= 0) t = t.slice(dotIdx + 1);
  ...
  if (parenIdx >= 0) t = t.slice(0, parenIdx).trim();

  // Collapse whitespace for multi-word types
  t = t.replace(/\s+/g, ' ');

  return PG_TYPES[t] ?? null;
}
