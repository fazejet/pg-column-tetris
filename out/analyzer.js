...
exports.simulateLayout = simulateLayout;
exports.resolveColumns = resolveColumns;
exports.optimalOrder = optimalOrder;
...
function simulateLayout(cols) {
    ...
    for (const col of cols) {
        const t = col.pgType;
        if (!t)
            continue; // unknown types contribute nothing to the analysis
        let pad;
...
function resolveColumns(cols) {
    return cols.map((c) => ({ ...c, pgType: (0, pgTypes_1.resolveType)(c.type) }));
}
/**
...
function analyzeTable(cols) {
    ...
    const interColPaddingDelta = Math.max(0, current.paddingBytes - optimal.paddingBytes);
